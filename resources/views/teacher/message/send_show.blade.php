            ...
            <div class="col-12">
                ...
                <div class="email-leftbar card">
                    ...
                    <div class="mail-list mt-4">
                        <a href="{{ route('teacher.message.index') }}"><i class="mdi mdi-email-outline me-2"></i> Inbox <span class="ms-1 float-end">({{ $recieve_message->count() }})</span></a>
                        <a href="{{ route('teacher.message.send') }}"  class="text-danger"><i class="mdi mdi-email-check-outline me-2"></i>Sent <span class="ms-1 float-end">({{ $m_t_teacher->count() }})</span></a>
                        ...
                        <div class="card-body">
                            <div class="media mb-4">
                                ...
                                <div class="media-body">
                                    ...
                                    @if($message->administrator=='recieve' and $message->student=='recieve')

                                    <h5 class="font-size-14 mt-1">{{  $message->administator->name }}, {{  $message->user->name }}</h5>
                                    <small class="text-muted">{{ $message->administator->email }}, {{ $message->user->email }}</small>

                                    @elseif ($message->administrator=='recieve' and $message->student=='0')

                                    <h5 class="font-size-14 mt-1">{{  $message->administator->name }}</h5>
                                    <small class="text-muted">{{ $message->administator->email }}</small>

                                    @elseif ($message->student=='recieve' and $message->administrator=='0')
